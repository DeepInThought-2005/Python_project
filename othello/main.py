...
class Bootstrap():
    ...
    def option_onclick(self):
        ...
        def on_save():
            ...
            if self.options.validStyleVar.get() == 'on':
                self.bC.valid_move_style = NUMBER
            elif self.options.validStyleVar.get() == 'off':
                self.bC.valid_move_style = DOT

            if self.options.isVmVar.get() == 'on':
                self.bC.is_draw_valid_moves = True
            elif self.options.isVmVar.get() == 'off':
