	...
	def space_pressed(self, event=None):
		...
		if not self.is_going:
			...
			if self.sim_winner != -1:
				print("获胜者： ", self.data[self.sim_winner])
				if self.times >= 12 and self.times < 15:
					if self.data[self.sim_winner] == "杨晶" or self.data[self.sim_winner] == "Michalle":
						print("los")
						rotate_times += 8
		...
		if self.is_going:
			...
			if self.step == 0:
				...
				if winner_ind != -1:
					...
					if self.times == 16:
						self.jiejiu = Label(self.win, text=jiejiu, font=("Times", 18, 'italic', 'bold'))
						self.jiejiu.place(x=1345, y=930)
						self.wiskey = Label(self.win, image=price[-1])
						self.wiskey.place(x=1360, y=760)
						self.gotdrunk_label = Label(self.win, image=price[-2])
						self.gotdrunk_label.place(x=1500, y=770)
